...
FUSION_PATTERN_REGISTRY: tuple[FusionPatternSpec, ...] = (
    ...
    FusionPatternSpec(
        ...
        # codespell:ignore-next-line thirdparty
        candidate_path=(
...
def extract_preferred_stack_location(stack: str | None) -> str | None:
    if not stack:
        return None
...
def infer_stage_from_annotation_name(name: str) -> str | None:
...
) -> dict[tuple[str, str], dict[float, FrameResolution | None]]:
    output: dict[tuple[str, str], dict[float, FrameResolution | None]] = {}
...
def resolve_kernel_site_context(
    ...
    frame_resolution_index: dict[tuple[str, str], dict[float, FrameResolution | None]]
    | None = None,
) -> tuple[str, str, str]:
...
def choose_mapping_frame(active_frames: Sequence[PythonFrame]) -> PythonFrame | None:
    if not active_frames:
        return None
...
def aggregate_kernel_sites(
    ...
    cpu_ops_by_external_id: dict[int, TimedEventIndex],
    python_frames: dict[tuple[str, str], list[PythonFrame]],
    launches_by_correlation: dict[int, TimedEventIndex] | None = None,
    site_context_cache: dict[
        tuple[str, str, float, int | None, int | None], tuple[str, str, str]
    ]
    | None = None,
...
def best_site_summary(kernel_entry: dict | None) -> tuple[str, str]:
    if not kernel_entry:
        return "unresolved", "-"
...
def entry_sites(kernel_entry: dict | None) -> list[dict]:
    if not kernel_entry:
        return []
    sites = kernel_entry.get("sites") or []
...
def kernel_entry_total_us(entry: dict | None) -> float:
    if not entry:
        return 0.0
    return sum(float(site.get("total_us", 0.0)) for site in entry.get("sites", []))


def kernel_entry_lookup_text(kernel_name: str, entry: dict | None) -> str:
    parts = [kernel_name]
    if entry:
...
def model_path_from_server_args(server_args: dict | None) -> str:
...
def pattern_supports_framework(spec: FusionPatternSpec, framework: str | None) -> bool:
...
def pattern_model_matches(spec: FusionPatternSpec, model_path: str) -> bool:
    if spec.model_include and not any(
        ...
        return False
    return not (
        spec.model_exclude and any(token in model_path for token in spec.model_exclude)
    )
...
def build_pattern_rationale(
    ...
    if spec.origin == "mainline":
        if has_active_match:
            return (
                f"`{spec.pattern}` is present in this trace "
                f"({share:.1f}% related GPU time). "
