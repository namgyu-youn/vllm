...
def build_triage_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Use either a single trace/profile input or a mapping+formal "
            "two-trace pair."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            "Repeat the word profiler many times with spaces so the server "
            "performs several decode steps. "
...
def pick_stage_value(stage_to_value: dict[str, object], stage: str) -> object | None:
...
def render_kernel_table_for_stage(rows: Sequence[dict]) -> list[str]:
    lines = [
        "| Kernel | Category | GPU time | Share | Launches "
        "| Python location (site share) | CPU op |",
        "| --- | --- | ---: | ---: | ---: | --- | --- |",
    ]
    if not rows:
        lines.append(
    ...
    for row in rows:
        lines.append(
            "| {kernel} | {category} | {gpu_time} | {share:.1f}% "
            "| {launches} | {location} | {cpu_op} |".format(
...
def render_overlap_table_for_stage(rows: Sequence[dict]) -> list[str]:
    lines = [
        "| Priority | Verdict | Kernel | Python scope | Formal signal "
        "| Dep risk | Recommendation |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    if not rows:
        lines.append(
            "| - | - | No rows cleared the 1.0% reporting bar. "
            "Use mapping/formal mode for overlap attribution. | - | - | - | - |"
        )
        return lines
    for row in rows:
        formal_signal = (
...
def render_fuse_table_for_stage(rows: Sequence[dict]) -> list[str]:
    lines = [
        "| Pattern | Confidence | Related GPU time | Share | Evidence kernels "
        "| Current kernel Python location | Candidate fused Python path "
        "| Rationale |",
        "| --- | --- | ---: | ---: | --- | --- | --- | --- |",
    ]
    if not rows:
        lines.append(
            "| No medium-confidence source-backed fusion opportunity "
            "matched this trace. | - | - | - | - | - | - | - |"
        )
        return lines
    for row in rows:
        lines.append(
            "| {pattern} | {confidence} | {gpu_time} | {share:.1f}% "
            "| {evidence} | {current_locations} | {candidate_path} "
            "| {rationale} |".format(
