...
def has_stream_marker(args: dict | None) -> bool:
    trace_args = args or {}
...
def load_server_args(path: Path) -> dict | None:
...
def try_get_json(url: str, timeout: float = 60.0) -> object | None:
    try:
...
def detect_framework_from_text(text: object) -> str | None:
...
def detect_framework_from_server_args(server_args: dict | None) -> str | None:
...
def detect_framework_from_trace(trace: object) -> str | None:
...
def detect_framework_from_path(path: Path) -> str | None:
...
def detect_framework_from_url(url: str, output_dir: str | None = None) -> str | None:
...
def parse_tp_rank(path: Path) -> int | None:
    for pattern in (
        r"(?:^|[_-])tp(\d+)(?:[_.-]|$)",
        r"TP-(\d+)",
        r"(?:^|[_-])rank(\d+)(?:[_.-]|$)",
...
def ensure_remote_profiler_output_path(output_dir: str | None, framework: str) -> Path:
    if not output_dir:
        raise ValueError(
...
def start_remote_profiler(
    url: str, framework: str, payload: dict | None = None
) -> None:
    try:
...
def run_profiler(
    ...
    probe_requests: int,
    probe_prompt: str,
    probe_max_new_tokens: int | None,
    probe_delay: float,
    warmup_steps: int = DEFAULT_WARMUP_STEPS,
    start_step: int | None = None,
    framework: str = "auto",
    framework_hint_path: str | None = None,
