...
Verified: unverified — no source diff on record yet; pending re-diff against
vLLM `main` (see the `Verified: <date> @ <sha>` convention in
`vllm-torch-compile-fusions.md`).

This catalog is the overlap-only companion to
`references/fuse-overlap-catalog.md`. The same vLLM scope note applies: rows
sourced from other frameworks are pattern references — re-verify the vLLM
equivalent before citing source in a report.
