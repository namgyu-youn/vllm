...
Verified: `2026-09-02` @ `b82f447d694d1d4280f068583f474fc52126ca57`.
Convention: this line records the last vLLM `main` commit this file was
source-diffed against — refresh it (date + sha) whenever the Pass
Registration or Pattern Inventory tables below are re-verified, even if
nothing changed.

Since the prior `2026-06-26` refresh (`abc71548ef0`), one new LLM
compile-fusion pass landed: `fuse_qk_norm_rope_kvcache` /
`QkNormRopeKvCacheFusionPass` (ROCm-only, `#42749`), now included below.
File paths in the Pattern Inventory were also corrected — passes moved to
`vllm/compilation/passes/fusion/`, one level deeper than previously listed.
...
`torch.compile` pattern matcher. Treat every row here as an existing precedent
before calling a similar fusion opportunity novel.
...
| `fuse_qk_norm_rope_kvcache` | `QkNormRopeKvCacheFusionPass` | ROCm-only; Q/K RMSNorm + RoPE + unified paged KV-cache update fused in one op, for supported head dims (64/128/256) |

## Pattern Inventory

Source files are under `vllm/compilation/passes/fusion/`.
...
| `rms_quant_fusion.py` | `RMSNormStaticQuantPattern`, `FusedAddRMSNormStaticQuantPattern`, `RMSNormDynamicQuantPattern`, `FusedAddRMSNormDynamicQuantPattern`, `RMSNormGroupQuantPattern`, `FusedAddRMSNormGroupQuantPattern` | RMSNorm or fused-add-RMSNorm followed by static FP8, dynamic per-token FP8, FP8 group quant, or NVFP4 quant | `_C.rms_norm_*_quant`, `_C.fused_add_rms_norm_*_quant`, or per-block quant custom op |
| `rocm_aiter_fusion.py` | `AiterRMSNormDynamicQuantPattern`, `AiterFusedAddRMSNormDynamicQuantPattern`, `AiterRMSFp8GroupQuantPattern`, `AiterFusedAddRMSFp8GroupQuantPattern` | AITER RMSNorm/fused-add-RMSNorm followed by AITER or vLLM FP8 quant | AITER fused RMSNorm-quant custom ops |
| `act_quant_fusion.py` | `SiluMulFp8StaticQuantPattern`, `SiluMulNvfp4QuantPattern`, `SiluMulBlockQuantPattern` | SiLU-and-mul activation output immediately quantized | fused activation-plus-quant custom op |
| `rocm_aiter_fusion.py` | `AiterSiluMulFp8GroupQuantPattern` | AITER SiLU-and-mul followed by FP8 group quant | AITER `act_mul_fused_fp8_group_quant` |
| `rocm_aiter_fusion.py` | `AddAiterRMSNormPadPattern` | AITER fused-add-RMSNorm output padded before the next op | AITER add-RMSNorm-pad op |
| `rocm_aiter_fusion.py` | `MLADualRMSNormPattern` | MLA Q branch and KV branch each run RMSNorm | `torch.ops.vllm.fused_mla_dual_rms_norm` backed by AITER fused QK RMSNorm |
| `qk_norm_rope_fusion.py` | `QkNormRopePattern` | Q/K RMSNorm, split/getitem reshapes, then RoPE | `_C.fused_qk_norm_rope` |
| `qk_norm_rope_kvcache_fusion.py` | `QkNormRopeKvCachePattern` | Q/K RMSNorm, split/getitem reshapes, RoPE, then `unified_kv_cache_update` (ROCm-only) | `torch.ops.vllm.fused_qk_norm_rope_and_unified_kv_cache_update` |
| `rope_kvcache_fusion.py` | `RopeReshapeKVCachePattern` | RoPE output followed by reshape/cache update | `vllm.fused_rope_and_unified_kv_cache_update` |
| `mla_rope_kvcache_cat_fusion.py` | `MLARoPEKVCacheCatPattern` | MLA RoPE on `q_pe` and `k_pe` flows into `unified_mla_kv_cache_update` | `vllm.fused_rope_unified_mla_kv_cache_update`, backed by `concat_and_cache_mla_rope_fused` |
| `attn_quant_fusion.py` | `AttnFp8StaticQuantPattern`, `AttnNvfp4QuantPattern` | attention output followed by FP8 static quant or NVFP4 quant | backend attention op with fused output quant when supported |
| `mla_attn_quant_fusion.py` | `MLAAttnFp8StaticQuantPattern`, `MLAAttnNvfp4QuantPattern`, `MLAAttnFp8GroupQuantPattern` | MLA attention output followed by static FP8, NVFP4, or FP8 group quant | MLA attention op with fused output quant when supported |
| `minimax_qk_norm_fusion.py` | `MiniMaxQKNormPattern` | MiniMax `forward_qk`: Q/K variance all-reduce divided by TP world size, then RMS apply | `vllm.minimax_qk_norm_fused` / Lamport fused kernel |
| `sequence_parallelism.py` | `FirstAllReduceRMSNormPattern`, `MiddleAllReduceRMSNormPattern`, `FirstAllReduceRMSNormStaticFP8Pattern`, `MiddleAllReduceRMSNormStaticFP8Pattern` | all-reduce plus norm block in a full-graph TP model | sequence-parallel reduce-scatter, local norm, all-gather staging |
...
- If the trace shows Q/K norm followed by RoPE or cache update, compare
  `QKNormRoPEFusionPass`, `RopeKVCacheFusionPass`, `QkNormRopeKvCacheFusionPass`
  (ROCm-only, combines both steps), and the MLA-specific
